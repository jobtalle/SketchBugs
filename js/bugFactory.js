const BugFactory = function() {
    this.makeBug = (random, right, x, y, motionOffset) => {
        let lastBug;
        let legs = true;
        ...
        const newBug = lastBug = new Bug(random, motionOffset, x, y, body, right, interleaveLegs ? (legs = (length & 1) === 1) : legs, null);
        const segments = [];
        ...
        for (let i = 0; i < length - 1; ++i) {
            ...
            const newSegment = new Bug(random, 0, 0, 0, body, right, legs, lastBug, body.getLength() * Bug.SEGMENT_OVERLAP);
